...
// Salt: Return average network hashes per second based on last number of blocks.
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid salt address");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid salt address:")+s.name_);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    CreateThread(Shutdown, NULL);
    return "wallet encrypted; salt server stopping, restart to run with encrypted wallet";
}
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Salt is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Salt is downloading blocks...");
...
//
// JSON-RPC protocol.  Salt speaks version 1.0 for maximum compatibility,
...
void ThreadRPCServer2(void* parg)
{
    ...
    ip::tcp::endpoint endpoint(bindAddress, GetArg("-rpcport", 58440));
...
#else
    if (fUseSSL)
        throw runtime_error("-rpcssl=1, but salt compiled without full openssl libraries.");
#endif
...
Object CallRPC(const string& strMethod, const Array& params)
{
    ...
    SSLIOStreamDevice d(sslStream, fUseSSL);
    iostreams::stream<SSLIOStreamDevice> stream(d);
    if (!d.connect(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", "58440")))
        throw runtime_error("couldn't connect to server");
#else
    if (fUseSSL)
        throw runtime_error("-rpcssl=1, but salt compiled without full openssl libraries.");

    ip::tcp::iostream stream(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", "58440"));
    if (stream.fail())
        throw runtime_error("couldn't connect to server");
#endif
